...
void test_Edge_should_beNone_when_intialized(void) {
    BUTTON_Initialize(&testButton);
    TEST_ASSERT_EQUAL_MESSAGE(EDGE_NONE, testButton.edge,
            "edge should be \"None\"");
}

void test_Edge_should_beRising_when_stateChangesFromLowToHigh(void) {
...
void test_Edge_should_beFalling_when_stateChangesFromHighToLow(void) {
...
void test_Edge_should_beNone_when_stateRemainsLow(void) {
    testButton.state = BUTTON_LOW;
...
void test_Edge_should_beNone_when_stateRemainsHigh(void) {
    testButton.state = BUTTON_HIGH;
